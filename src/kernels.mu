...
/**
 * @brief Kernel to compute the trace of a matrix.
 * 
...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols) {
  ...
  RUNTIME_CHECK(musaMalloc(&d_input, input_size));
  
  // Copy input data from host to device
  RUNTIME_CHECK(musaMemcpy(d_input, h_input.data(), input_size, musaMemcpyHostToDevice));
  
  // Allocate device memory for result and initialize to zero
  T* d_result;
  RUNTIME_CHECK(musaMalloc(&d_result, sizeof(T)));
  RUNTIME_CHECK(musaMemset(d_result, 0, sizeof(T)));
  ...
  T h_result;
  RUNTIME_CHECK(musaMemcpy(&h_result, d_result, sizeof(T), musaMemcpyDeviceToHost));
  
  // Free device memory
  RUNTIME_CHECK(musaFree(d_input));
  RUNTIME_CHECK(musaFree(d_result));
...
/**
 * @brief Kernel for Flash Attention
 * 
...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
  ...
  RUNTIME_CHECK(musaMalloc(&d_k, k_size));
  RUNTIME_CHECK(musaMalloc(&d_v, v_size));
  RUNTIME_CHECK(musaMalloc(&d_o, o_size));
  
  // Copy data from host to device
  RUNTIME_CHECK(musaMemcpy(d_q, h_q.data(), q_size, musaMemcpyHostToDevice));
  RUNTIME_CHECK(musaMemcpy(d_k, h_k.data(), k_size, musaMemcpyHostToDevice));
  RUNTIME_CHECK(musaMemcpy(d_v, h_v.data(), v_size, musaMemcpyHostToDevice));
  ...
  // Shared memory: query (head_dim) + output (head_dim) + reduce buffer (threads_per_block) + max_score (1) + sum_exp (1)
  size_t shared_mem_size = (head_dim * 2 + threads_per_block + 2) * sizeof(float);
